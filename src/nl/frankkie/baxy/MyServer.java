...
import nl.frankkie.baxy.web.AppImage;
import nl.frankkie.baxy.web.Apps;
import nl.frankkie.baxy.web.Error404;
import nl.frankkie.baxy.web.GetApps;
import nl.frankkie.baxy.web.GetDirectory;
import nl.frankkie.baxy.web.Index;
import nl.frankkie.baxy.web.InstallAPK;
import nl.frankkie.baxy.web.SetBackground;
import nl.frankkie.baxy.web.StartApp;
import nl.frankkie.baxy.web.UninstallApp;
import nl.frankkie.baxy.web.Upload;
import nl.frankkie.baxy.web.WebPage;
import nl.frankkie.baxy.web.WebUtil;

public class MyServer extends NanoHTTPD {

    public static WebserverActivity webserverActivity;

    public static void log(String s) {
        if (webserverActivity != null) {
            webserverActivity.log(s);
        }
    }

    private static final Map<String, Class<? extends WebPage>> webpages = new HashMap<String, Class<? extends WebPage>>() {
        {
            put("/index", Index.class);
            put("/", Index.class);
            put("/upload", Upload.class);
            put("/error404", Error404.class);
            put("/installApk", InstallAPK.class);
            put("/getApps", GetApps.class);
            put("/appImage", AppImage.class);
            put("/apps", Apps.class);
        ...
        if (up != null || directories.size() + files.size() > 0) {
            ...
            if (files.size() > 0) {
                ...
                for (int i = 0; i < files.size(); i++) {
                    ...
                    msg += "<li><a href=\"/files" + encodeUri(uri + file) + "\"><span class=\"filename\">" + file + "</span></a>";
    ...
    public Response serve(String uri, Method method, Map<String, String> header, Map<String, String> parms, Map<String, String> files) {
        ...
        WebPage page = null;
        try {
            Class<? extends WebPage> aClass = webpages.get(uri);
            if (aClass == null) {
                page = new Error404();
            } else {
                page = aClass.newInstance();
            }
            response = page.serve(uri, method, header, parms, files);
        } catch (Exception e) {
            e.printStackTrace();
            response = new Response(Response.Status.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "Error 500, server error.\n" + e);
        }
        return response;
    }
     ...
     */
    public static void main(WebserverActivity webserverActivityInstance) {
        // Defaults
        int port = 1234;
        webserverActivity = webserverActivityInstance;

        String host = "127.0.0.1";
        File wwwroot = new File("/sdcard/").getAbsoluteFile();
        boolean quiet = false;
...
//        }
        ...
        ServerRunner.executeInstance(new MyServer(host, port, wwwroot, quiet));
    }
}
